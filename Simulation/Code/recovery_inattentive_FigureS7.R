...
run_stan <- function(iter_sampling = 2,  # DEMO use 2000 for full fit
                     ...
                     chains          = 4,
                     parallel_chains = 4) {
  load("Simulation/Data/Random_responding/Recovery/simulated_standata.rdata")
  model_path  <- "Simulation/Code/Stan_models/full_model.stan"
  output_name <- "Simulation/Output/random_responding/recovery/modelfit_random_responding_recovery.rds"
...
dir.create("Simulation/Output/random_responding/recovery/plots", recursive = TRUE, showWarnings = FALSE)

fit <- run_stan()

# Figure S7 — posterior lambda (random responding recovery) ---------------
...
p_s7 <- ggplot(df_plot, aes(x = lambda, y = 0)) +
...
ggsave("Simulation/Output/random_responding/recovery/plots/figureS7_lambda_posterior.pdf",
       p_s7, width = 6, height = 3)
