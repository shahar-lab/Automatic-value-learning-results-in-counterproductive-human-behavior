data {
 ...
 int<lower=1> Ndata;      // Total number of trials (for all subjects)
  ...
  array[Ndata] int<lower=0> ch_card; //index of which card was chosen coded 1 to 2
  ...
  array[Ndata] int<lower=0> key1; //offered location first card (1-4)

  array[Ndata] int<lower=0> key2; //offered location second card(1-4)
...
transformed parameters {
 ...
 decay_explore_key_sbj = inv_logit(mu_decay_explore_key + sigma_decay_explore_key * decay_explore_key_raw);//transformed to 0-1
  ...
  vector [Nraffle] Q_cards; //vector of values for the 4 cards
  vector [Narms] Q_keys; // vector of values for the four keys
  vector [Nraffle] Q_keys_offered; //vector of values for the 2 currently offered keys
  
  vector [Nraffle] E_cards; //vector of accumulated exploration/perseveration card tendencies
  vector [Narms] E_keys; //vector of accumulated exploration/perseveration key tendencies
  vector [Nraffle] E_keys_offered;
  ...
  if (first_trial_in_block[t] == 1) {
    //reset all values in a new block
      Q_cards=rep_vector(0.5, Nraffle);
      Q_keys=rep_vector(0.5, Narms);
      
      E_cards=rep_vector(0, Nraffle);
      E_keys=rep_vector(0, Narms);
    }
          Q_keys_offered[1]=Q_keys[key1[t]]; //key 1 is the location of card1
          Q_keys_offered[2]=Q_keys[key2[t]]; //key 2 is the location of card2
          
          E_keys_offered[1]=E_keys[key1[t]];
          E_keys_offered[2]=E_keys[key2[t]];
          
          //integrating all values to a net value
          Qnet=beta_t*(lambda_t*Q_keys_offered+(1-lambda_t)*Q_cards)+E_keys_offered+E_cards;
