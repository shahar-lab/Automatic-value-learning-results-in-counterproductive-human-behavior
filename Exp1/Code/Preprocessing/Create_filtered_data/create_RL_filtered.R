rm(list = ls())
load(file="data/data_raw/RL_raw.rdata")
library(tidyverse)
#filter
...
df <- unique(RL_raw$subject_id) %>%
  lapply(function(subject_id) filter_subject_id_data(subject_id, RL_raw)) %>%
  bind_rows()



#count removed trials per sample
ntrials_before=RL_raw%>%group_by(sample)%>%summarise(n())
