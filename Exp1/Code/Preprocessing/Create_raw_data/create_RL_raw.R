...
process_RL <- function(file_path) {
  ...
  instructions_mistakes
  clarifications_mistakes <- sum(str_detect(df$stimulus, "Let`s go back to the instructions."), na.rm = TRUE)
  ...
  df=df%>%filter(trial_name=="reward2")%>%
    ...
    mutate(block=factor(block+1),trial=factor(trial+1),rt=rt,
           ...
           prev_loc_better=if_else(lag(ch_key)==better_loc,1,0),
                                   ...
                                   response_clear_easy=as.numeric(responses$response[1]),
                                   response_fair_honest=as.numeric(responses$response[2]),
                                   response_think_locs=as.numeric(responses$response[3]),
                                   response_tricked_manipulated=as.numeric(responses$response[4]),
                                   response_use_locs=as.numeric(responses$response[5]))
  
  return (df)
}
# Get all CSV files in the directory
files <- list.files("data/data_collected/story/RL", pattern = "\\.csv$", full.names = TRUE)
# Process each file and combine results into a dataframe
RL_raw_story <- do.call(rbind, lapply(files, process_RL))%>%mutate(sample="story")
#save(RL_raw_story,file="data/data_raw/story/RL_raw.rdata")
files <- list.files("data/data_collected/abstract/RL", pattern = "\\.csv$", full.names = TRUE)
# Process each file and combine results into a dataframe
RL_raw_abstract <- do.call(rbind, lapply(files, process_RL))%>%mutate(sample="abstract")
#save(RL_raw_abstract,file="data/data_raw/abstract/RL_raw.rdata")
#combine
RL_raw=rbind(RL_raw_story,RL_raw_abstract)%>%mutate(subject=as.integer(factor(subject_id)),
                                                    sample=factor(sample,levels=c("abstract","story")))%>%arrange(subject)%>%
                                                    select(subject,subject_id,sample, everything())

save(RL_raw,file="data/data_raw/RL_raw.rdata")

#save without subject_id in csv
write.csv(RL_raw_story%>%select(-subject_id),file="data/data_raw/story/RL_raw.csv")
