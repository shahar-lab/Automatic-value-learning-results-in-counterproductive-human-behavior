...
process_RL <- function(file_path) {
  ...
  instructions_mistakes <- sum(str_detect(df$stimulus, "Let`s go back to the instructions."), na.rm = TRUE)
  illustration_oil_mistakes <- sum(str_detect(df$stimulus, "only the shape you choose is related to your chances of winning a coin"), na.rm = TRUE)
  ...
  df=df%>%filter(trial_name=="reward2")%>%
    ...
    mutate(block=block+1,rt=rt,ch_key=as.integer(ch_key),
           ...
           illustration_oil_mistakes=illustration_oil_mistakes,
